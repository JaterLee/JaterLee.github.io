/**
 * sync-qin-ershi.js
 * 将 content/qin-ershi/*.md 编译为 data/qin-ershi-notes.json
 * 「秦二世必须死」— 连载小说章节
 *
 * .md 文件格式：
 *   ---
 ...
 *   正文（Markdown）...
 *
 * 用法：node scripts/sync-qin-ershi.js
 */
...
const CONTENT_DIR = path.join(PROJECT_ROOT, 'content', 'qin-ershi');
const OUTPUT_PATH = path.join(PROJECT_ROOT, 'data', 'qin-ershi-notes.json');
...
async function build() {
  if (!fs.existsSync(CONTENT_DIR)) {
    ...
    console.log('📁 已创建 content/qin-ershi/ 目录');
  }
  ...
  if (mdFiles.length === 0) {
    console.log('⚠️  content/qin-ershi/ 中没有 .md 文件');

    if (fs.existsSync(OUTPUT_PATH)) {
      console.log('    data/qin-ershi-notes.json 保持不变');
    } else {
