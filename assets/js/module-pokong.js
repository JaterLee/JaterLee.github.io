/**
 * module-pokong.js
 * 破空：春秋异客传 Module — 章节卡片瀑布流 + 筛选 + 详情弹窗 + markdown 渲染
...
(function () {
  ...
  function renderMarkdown(md) {
    ...
    return '<div class="pokong-md-body">\n' + blocks.join('\n') + '\n</div>';
  }
  ...
  function updateLikeDisplay(card, count) {
    var btn = card.querySelector('.pokong-like-btn');
    if (btn) {
      btn.innerHTML = count > 0
        ? '<span class="pokong-like-heart liked">❤️</span><span class="pokong-like-count">' + count + '</span>'
        : '<span class="pokong-like-heart">🤍</span>';
    }
    card.classList.toggle('pokong-card-rainbow', count >= STATE.likeThreshold);
  }
  ...
  async function loadNotes() {
    ...
    try {
      var resp = await fetch('data/pokong-notes.json', { signal: controller.signal });
      ...
      clearTimeout(timeoutId);
      console.warn('Pokong module: failed to load', err.message);
      STATE.notes = [];
      renderAll();
    }
  }
  ...
  function renderStats() {
    var elTotal = $('#stat-pokong-notes');
    var elRange = $('#stat-pokong-range');
  ...
  function typeBadge(type) {
    if (type === '番外') return '<span class="pokong-type-badge pokong-type-side">番外</span>';
    if (type === '设定') return '<span class="pokong-type-badge pokong-type-setting">设定</span>';
    return '<span class="pokong-type-badge pokong-type-main">正文</span>';
  }
  ...
  function renderCards(container, notes, startIdx) {
    ...
    var html = notes.map(function (note) {
      ...
      var chapterRow = '';
      if (note.chapter) {
        chapterRow = '<div class="pokong-card-chapter">' + escapeHtml(note.chapter) + '</div>';
      }
      var tagsHTML = '';
      if (note.tags && note.tags.length) {
        tagsHTML = '<div class="pokong-card-tags">' +
          note.tags.map(function (t) { return '<span class="pokong-card-tag">#' + escapeHtml(t) + '</span>'; }).join('') +
        '</div>';
      }
      return '<article class="pokong-card" data-note-id="' + escapeHtml(note.id) + '" tabindex="0">' +
        typeBadge(note.type) +
        '<h3 class="pokong-card-title">' + escapeHtml(note.title) + '</h3>' +
        chapterRow +
        '<p class="pokong-card-excerpt">' + escapeHtml(excerpt) + '</p>' +
        tagsHTML +
        '<div class="pokong-card-footer">' +
          '<button class="pokong-like-btn" data-note-id="' + escapeHtml(note.id) + '">' +
            '<span class="pokong-like-heart">🤍</span>' +
          '</button>' +
          '<span class="pokong-card-date">' + formatDate(note.date) + '</span>' +
          '<span class="pokong-card-detail-btn">阅读 →</span>' +
        '</div>' +
      '</article>';
    }).join('');
    ...
    if (startIdx === 0) {
      ...
      var existingBtn = container.querySelector('.pokong-load-more');
      if (existingBtn) existingBtn.remove();
      container.insertAdjacentHTML('beforeend', html);
    }

    container.querySelectorAll('.pokong-card').forEach(function (card) {
      ...
      var likeBtn = card.querySelector('.pokong-like-btn');
      if (likeBtn) {
  ...
  function renderLoadMoreButton(container, remaining) {
    ...
    container.insertAdjacentHTML('beforeend',
      '<div class="pokong-load-more-wrap"><button class="pokong-load-more" id="pokong-load-more">' +
      '加载更多 <span class="pokong-load-more-count">（还有 ' + remaining + ' 章）</span></button></div>'
    );
    var btn = $('#pokong-load-more');
  ...
  function renderMasonry() {
    ...
    if (!filtered.length) {
      dom.masonry.innerHTML = '<div class="pokong-empty-filter"><p>没有符合条件的章节</p></div>';
  ...
  function openModal(noteId) {
    ...
    if (dom.modalTypeBadge) {
      ...
      dom.modalTypeBadge.className = 'pokong-modal-type-badge' +
        (note.type === '番外' ? ' pokong-type-side' : note.type === '设定' ? ' pokong-type-setting' : ' pokong-type-main');
    }
    if (dom.modalMeta) {
      var metaParts = [];
      if (note.chapter) metaParts.push('<div class="pokong-modal-chapter">📖 ' + escapeHtml(note.chapter) + '</div>');
      metaParts.push('<div class="pokong-modal-date">📅 ' + formatDate(note.date) + '</div>');
      if (note.tags && note.tags.length) {
        metaParts.push('<div class="pokong-modal-tags">' +
          note.tags.map(function (t) { return '<span class="pokong-modal-tag">#' + escapeHtml(t) + '</span>'; }).join('') +
        '</div>');
      }
  ...
  function init() {
    ...
    dom = {
      loading: $('#pokong-loading'),
      empty: $('#pokong-empty'),
      masonry: $('#pokong-masonry'),
      filterBtns: document.querySelectorAll('.pokong-filter-btn'),
      modalOverlay: $('#pokong-modal-overlay'),
      modal: document.querySelector('.pokong-modal'),
      modalTitle: $('#pokong-modal-title'),
      modalTypeBadge: $('#pokong-modal-type-badge'),
      modalMeta: $('#pokong-modal-meta'),
      modalBody: $('#pokong-modal-body'),
      modalClose: $('#pokong-modal-close'),
      modalCloseBottom: $('#pokong-modal-close-bottom'),
    };
    STATE.loaded = true;
    bindEvents();
    loadNotes();
  ...
  if (window.JaterMod) {
    window.JaterMod.register('pokong', { init: init });
  } else {
    document.addEventListener('DOMContentLoaded', function () {
      if (window.JaterMod) window.JaterMod.register('pokong', { init: init });
    });
  }
})();
